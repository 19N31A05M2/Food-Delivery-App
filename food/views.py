...
def restaurant(request):
    ...
    categories=Product.objects.filter(Restaurent_s_id=res_id).values('category').distinct()
    #print(res_products)
    if request.session.test_cookie_worked():
        user['name']=request.COOKIES['name']
        user['customer_id']=request.COOKIES['customer_id']
        user['number']=request.COOKIES['number']
    return render(request,'restaurant.html',{'products':res_products,'user':user,'restaurentdetails':restuarant_details,'categories':categories})
